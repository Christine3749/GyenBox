...
import { getCurrentMembershipEntitlements, MembershipRequiredError } from "@/lib/membership"
...
export async function POST(request: Request) {
  ...
  const configuredMaxUploadBytes = Number(process.env.MAX_UPLOAD_BYTES ?? 5 * 1024 * 1024 * 1024)
  const maxUploadBytes = entitlements?.maxFileSizeBytes && entitlements.maxFileSizeBytes > 0
  ...
  const fileId = typeof fileIdValue === "string" && fileIdValue ? fileIdValue : null
  ...
  try {
    ...
    const storageQuota = entitlements ? BigInt(entitlements.storageQuotaBytes) : user.storageQuota
    const projectedStorage = user.storageUsed - (currentFile?.size ?? BigInt(0)) + BigInt(uploaded.size)
    ...
    const file = await prisma.$transaction(async (tx) => {
      if (currentFile) {
        ...
        await tx.user.update({
          ...
          data: { storageUsed: { increment: BigInt(uploaded.size) - currentFile.size } },
        })
        return updated
      }
...
async function readUploadEntitlements(request: Request) {
  const accessToken = getBearerToken(request)
  if (!accessToken) return { entitlements: null, response: null }

  try {
    return { entitlements: await getCurrentMembershipEntitlements(accessToken), response: null }
  } catch (error) {
    if (error instanceof MembershipRequiredError) {
      return {
        entitlements: null,
        response: fail("MEMBERSHIP_REQUIRED", "Activate a GyenBox membership before uploading.", 403),
      }
    }

    return {
      entitlements: null,
      response: fail("MEMBERSHIP_UNAVAILABLE", "Membership is not ready yet.", 503, {
