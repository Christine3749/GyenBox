...
function formatStorageFromBytes(bytes: number) {
  const gb = bytes / 1_073_741_824
  return gb >= 1 ? gb.toFixed(1) : (bytes / 1_048_576).toFixed(1)
}
...
export default function GyenboxWorkspace() {
  ...
  function toggleStar(file: FileItem) {
    void patchItem(file, { isStarred: !file.starred }, file.starred ? 'Removed from starred' : 'Added to starred')
  }

  function moveToTrash(file: FileItem) {
    void patchItem(file, { isTrashed: true }, 'Moved to trash')
    setSelectedId(null)
  }
  ...
  if (authStatus === 'loading' || authStatus === 'unauthenticated') {
    return <StatusScreen title="Connecting GyenBox" detail="Checking your Supabase session." />
  }

  if (authStatus === 'unconfigured') {
    return <StatusScreen title="Supabase key missing" detail="Set NEXT_PUBLIC_SUPABASE_ANON_KEY before using production login." />
  }
    ...
    <div className="flex h-screen overflow-hidden bg-[#08090B] text-[#F4F1EA]">
      ...
      <aside className="relative z-10 flex w-[248px] shrink-0 flex-col border-r border-white/10 bg-[#0B0E13]/95 px-3 py-4">
        ...
        <div className="mt-6 rounded-md border border-white/10 bg-white/[0.03] p-3">
          <div className="mb-3 flex items-center justify-between text-xs text-[#89919F]">
            ...
            <span className="font-mono">
              {formatStorageFromBytes(storageUsedBytes)} {storageUnit(storageUsedBytes)} / {formatStorageFromBytes(storageQuotaBytes)} {storageUnit(storageQuotaBytes)}
            </span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/10">
            <div className="h-full rounded-full bg-[#8896C6]" style={{ width: `${storagePercent}%` }} />
          </div>
        ...
        <div className="mt-auto rounded-md border border-white/10 bg-[#8896C6]/10 p-3 text-xs leading-5 text-[#DDE4FF]">
          ...
          <p className="text-[#AEB9DF]">Supabase Auth, Postgres metadata, and Google Cloud Storage objects.</p>
        </div>
      </aside>
      ...
      <main className="relative z-10 flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 shrink-0 items-center gap-3 border-b border-white/10 bg-[#0B0E13]/80 px-5 backdrop-blur">
          ...
          <button
            className="flex h-9 items-center gap-2 rounded-md bg-[#8896C6] px-3 text-sm font-semibold text-[#0A0E14] hover:bg-[#7B89BD] disabled:opacity-60"
          ...
          <button className="flex h-9 items-center gap-2 rounded-full bg-[#1B202B] px-3 text-xs font-semibold text-[#F4F1EA]" onClick={signOut} title="Sign out">
            <span>{accountLabel}</span>
            <LogOut className="h-3.5 w-3.5 text-[#89919F]" />
          </button>
        </header>

        <section className="grid shrink-0 grid-cols-1 gap-3 border-b border-white/10 bg-[#08090B]/65 p-5 md:grid-cols-3">
          <Metric icon={HardDrive} label="Storage used" value={`${formatStorageFromBytes(storageUsedBytes)} ${storageUnit(storageUsedBytes)}`} detail="Google Cloud Storage" />
        ...
        <section className="flex min-h-0 flex-1">
          <div className="flex min-w-0 flex-1 flex-col">
            <div className="flex shrink-0 flex-wrap items-center gap-2 px-5 py-4">
              ...
              <button className="flex h-8 items-center gap-2 rounded-md border border-white/10 px-2.5 text-xs font-medium text-[#AAB2C0] hover:bg-white/[0.04] hover:text-white" onClick={createFolder}>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-5">
              {isLoadingFiles ? (
                <div className="flex h-full items-center justify-center rounded-md border border-dashed border-white/10 text-sm text-[#7E8796]">Loading files...</div>
              ) : visibleFiles.length === 0 ? (
                <div className="flex h-full items-center justify-center rounded-md border border-dashed border-white/10 text-sm text-[#7E8796]">No files in this view.</div>
              ) : viewMode === 'grid' ? (
                <div className="grid grid-cols-[repeat(auto-fill,minmax(190px,1fr))] gap-3">
                  {visibleFiles.map((file) => (
                    <FileTile key={file.id} file={file} selected={file.id === selectedId} onOpen={handleOpen} onSelect={() => setSelectedId(file.id)} onShare={() => setShareFile(file)} onStar={() => toggleStar(file)} />
                  ))}
                </div>
              ) : (
                <div className="overflow-hidden rounded-md border border-white/10 bg-[#0B0E13]/70">
                  {visibleFiles.map((file) => (
                    <FileRow key={file.id} file={file} selected={file.id === selectedId} onOpen={handleOpen} onSelect={() => setSelectedId(file.id)} onShare={() => setShareFile(file)} onStar={() => toggleStar(file)} />
                  ))}
                </div>
              )}
            </div>
...
function StatusScreen({ title, detail }: { title: string; detail: string }) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#08090B] px-6 text-[#F4F1EA]">
      <section className="w-full max-w-sm rounded-md border border-white/10 bg-[#0B0E13] p-5 text-center shadow-2xl">
        <GyenBoxLogo showSubtitle markClassName="mx-auto h-10 w-10" />
        <h1 className="mt-6 text-lg font-semibold">{title}</h1>
        <p className="mt-2 text-sm leading-6 text-[#89919F]">{detail}</p>
      </section>
    </main>
  )
}
